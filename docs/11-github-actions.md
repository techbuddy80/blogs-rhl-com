...
**Step 12: Deployment** — Node process management (PM2 or systemd) instructions for running the built site in the homelab, Cloudflare Tunnel configuration for exposing `blogs.ramhl.com`, DNS setup, and wiring the CI workflow to actually deploy on merge to `main` rather than just validating.
